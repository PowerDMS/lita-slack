  ...
  module Adapters
    class Slack < Adapter
      ...
      class ChatService
        ...
        # @return [void]
        def send_file_content(content, filename, filetype = "text", title = nil, initial_comment = nil, channels = nil)
          api.send_file_content(content, filename, filetype, title, initial_comment, channels)
        end
      end
    end
  end
